"""
Скрипт для точной синхронизации покупок с 1С
Удаляет все покупки из БД и импортирует только те, что есть в 1С
"""
import asyncio
import os
import sys
...
async def sync_with_1c(phone: str):
    """Точная синхронизация покупок с 1С"""
    ...
    async with AsyncSessionLocal() as db:
        ...
        print("=" * 80)
        print(f"ТОЧНАЯ СИНХРОНИЗАЦИЯ С 1С: {phone}")
        print("=" * 80)
        print()
        print(f"Покупатель: {user.full_name}")
        print()
        
        # 2. Получаем все покупки из 1С
...
def main():
    ...
    print()
    asyncio.run(sync_with_1c(phone))
