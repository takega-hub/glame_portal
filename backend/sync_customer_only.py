...
async def sync_customer_only(phone: str):
    """Синхронизация ТОЛЬКО покупок данного покупателя"""
    ...
    async with AsyncSessionLocal() as db:
        ...
        # 2. Получаем покупки из 1С ТОЛЬКО по дисконтной карте
        print("Получение покупок из 1С по дисконтной карте...")
        ...
        # Используем регистр продаж по дисконтным картам
        url = f"{api_url.rstrip('/')}/AccumulationRegister_ПродажиПоДисконтнымКартам_RecordType"
                    ...
                    if response.status_code == 200:
                        ...
                        for record in records:
                            ...
                            for movement in record_set:
                                discount_card_key = movement.get("ДисконтнаяКарта_Key", "")
                                
                                # Фильтруем ТОЛЬКО по дисконтной карте этого покупателя
                                if discount_card_key == user.discount_card_id_1c:
        ...
        print(f"Всего покупок в 1С (по дисконтной карте): {len(all_purchases_1c)}")
...
def main():
    ...
    print()
    asyncio.run(sync_customer_only(phone))
