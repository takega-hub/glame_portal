"""
Скрипт для импорта ВСЕХ покупок из 1С в БД для покупателя 79787566405
"""
import asyncio
import os
import sys
...
async def import_all_purchases(phone: str):
    """Импорт ВСЕХ покупок из 1С в БД"""
    ...
    async with AsyncSessionLocal() as db:
        ...
        print("=" * 80)
        print(f"ИМПОРТ ВСЕХ ПОКУПОК ИЗ 1С В БД: {phone}")
        print("=" * 80)
        print()
        ...
        # 2. Получаем покупки из 1С напрямую (ВСЕ покупки)
        print("Получение ВСЕХ покупок из 1С...")
            ...
            try:
                async with httpx.AsyncClient(timeout=120.0) as client:
                    ...
                    if response.status_code == 200:
                        ...
                        if len(all_purchases) >= 20000:
                            print(f"  Достигнут лимит 20000 покупок, завершаем")
                            break
        ...
        print(f"Существует {len(existing_purchases)} покупок в БД")
        print()
...
def main():
    ...
    print()
    asyncio.run(import_all_purchases(phone))
