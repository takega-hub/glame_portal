# Деплой GLAME AI на portal.glamejewelry.ru

Пошаговая инструкция по развёртыванию платформы на VPS с Docker и переносу данных с локальной Windows-машины.
...
Скрипт сам сделает `git pull`, пересборку контейнеров (`docker compose up -d --build`) и миграции БД (`alembic upgrade head`). Перед первым запуском сделайте `chmod +x scripts/deploy_from_git.sh`.
...
После копирования выполните на сервере шаги из раздела [Восстановление данных на сервере](#восстановление-данных-на-сервере).
...
# Скопировать в тома (Docker хранит тома в каталоге, например)
# Проще: запустить временный контейнер с монтированием тома и скопировать туда файлы из архива.
...
Подробности в разделе [Бэкап и восстановление Qdrant](#бэкап-и-восстановление-qdrant).
...
См. также общий [DEPLOY.md](../DEPLOY.md) и [README.md](../README.md).
