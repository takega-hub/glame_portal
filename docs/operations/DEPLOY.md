...
```bash
cd infra
docker run --rm -it -v "$(pwd):/data" -w /data httpd:alpine htpasswd .htpasswd admin
```

Команда обновит запись пользователя `admin` в `infra/.htpasswd` (без удаления других пользователей, если они есть). Затем перезапустите nginx:
