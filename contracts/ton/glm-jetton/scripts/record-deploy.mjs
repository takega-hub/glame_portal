...
function main() {
  ...
  const artifactPath = path.resolve(process.cwd(), args.artifact || 'glm-jetton.testnet.json');
  const artifact = JSON.parse(fs.readFileSync(artifactPath, 'utf8'));
  const network = nonEmpty(args.network) || process.env.TON_NETWORK || artifact.network;
  if (network !== 'testnet') {
    console.error('Refusing to record non-testnet deployment in this package.');
    process.exit(2);
  }
  ...
  const adminAddress = nonEmpty(args.adminAddress) || nonEmpty(process.env.TON_JETTON_ADMIN_ADDRESS);
  const treasuryAddress = nonEmpty(args.treasuryAddress) || nonEmpty(process.env.TON_GLM_TREASURY_ADDRESS);
  ...
  if (!treasuryAddress) errors.push('Missing --treasury-address or TON_GLM_TREASURY_ADDRESS');
  if (errors.length) {
