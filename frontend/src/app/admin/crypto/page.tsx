...
type GlmTreasuryTurnover = {
  ...
  period_days: number;
  since: string;
  operational_stats_start_at?: string | null;
  generated_at: string;
  incoming_glm: number;
  outgoing_glm: number;
  net_glm: number;
...
type TonConnectTransactionPayload = {
  ...
  approval?: GlmHotWalletRefillCheck | null;
  transaction: {
    validUntil: number;
    network?: string;
    from?: string;
...
type GlmTonReadiness = {
  ...
  security?: {
    ...
    production_signer_pause_endpoint_configured?: boolean;
    production_signer_auth_configured?: boolean;
  ...
  go_no_go?: {
    ...
    ready?: boolean;
    checked_at?: string;
    max_amount_glm?: number;
    daily_limit_glm?: number;
    checks?: Array<{ code: string; ok: boolean; label?: string; message: string }>;
    blockers?: Array<{ code: string; ok: boolean; label?: string; message: string }>;
    next_steps?: string[];
  };
  alerts?: Array<{ code: string; severity?: string; message: string }>;
...
type ProductionSignerCheck = {
  configured?: boolean;
  mode?: string | null;
  endpoint_configured?: boolean;
  health_endpoint_configured?: boolean;
  ...
  emergency_pause_source?: string | null;
  emergency_pause_reason?: string | null;
  emergency_pause_updated_at?: string | null;
  emergency_pause_updated_by?: string | null;
  errors?: string[];
};

type GlmReplayIdempotencyAudit = {
  schema?: string;
  status?: string;
  checked_at?: string;
  checked?: {
...
type GlmEffectiveness = {
  generated_at: string;
  network?: string;
  period: { current_month_start: string; operational_stats_start_at?: string | null };
  accounts_total: number;
...
const statusLabels: Record<string, string> = {
  ...
  automatic_mainnet_hot_wallet_transfer: 'Mainnet auto-transfer',
  production_ready: 'Production ready',
  legal_security_treasury_or_signer_approval_required: 'Approval required',
  ...
  two_step_required: 'Нужен approval',
  balance_check: 'Проверка баланса',
  manual_refill: 'Ручное пополнение',
  refill_approval: 'Approval refill',
  incoming: 'Входящий',
  outgoing: 'Исходящий',
...
function tonReadinessTitle(network?: string | null) {
  const normalized = String(network || 'testnet').trim().toLowerCase();
  return `TON ${normalized === 'mainnet' ? 'Mainnet' : 'Testnet'} Readiness`;
}

function jettonDeploymentLabel(status?: string | null, network?: string | null) {
  const normalizedNetwork = String(network || '').trim().toLowerCase();
  const normalizedStatus = String(status || '').trim();
  if (normalizedStatus === 'testnet_deployed' && normalizedNetwork === 'mainnet') {
    return 'Mainnet deployed';
  }
  return label(normalizedStatus || 'missing');
}
...
export default function AdminReferralsPage() {
  ...
  const hotWalletRefillApprovalRequired = Boolean(hotWalletRefillPlan?.approval_required);
  const hotWalletRefillCheckMatchesPlan = (item: GlmHotWalletRefillCheck) => {
    if (!hotWalletRefillPlan) return false;
    const sameGlm = Number(item.refill_glm_amount || 0) === Number(hotWalletRefillPlan.refill_glm_amount || 0);
    const sameTon = Number(item.refill_ton_amount || 0) === Number(hotWalletRefillPlan.refill_ton_amount || 0);
    return (
      sameGlm
      && sameTon
      && String(item.network || '') === String(hotWalletRefillPlan.network || '')
      && String(item.treasury_address || '') === String(hotWalletRefillPlan.source_address || '')
      && String(item.hot_wallet_address || '') === String(hotWalletRefillPlan.destination_address || '')
    );
  };
  const hotWalletPendingRefillApprovals = hotWalletRefillChecks.filter((item) => (
    item.event_type === 'refill_approval' && item.status === 'pending' && hotWalletRefillCheckMatchesPlan(item)
  ));
  const hotWalletMatchingApprovedRefillApproval = hotWalletRefillChecks.find((item) => (
    item.event_type === 'refill_approval' && item.status === 'approved' && hotWalletRefillCheckMatchesPlan(item)
  ));
  ...
  async function loadGlmTonReadiness() {
    ...
    if (security) {
      setProductionApprovalForm((prev) => ({
        legal_approved: Boolean(security.production_legal_approved),
  ...
  async function checkProductionSigner() {
    ...
    setError(null);
    try {
      const response = await apiClient.post<ProductionSignerCheck>('/api/referrals/admin/glm-production-signer/check');
      setProductionSignerCheck(response.data);
    } catch (e: any) {
      setError(e?.response?.data?.detail || e?.message || 'Не удалось проверить production signer');
    } finally {
      setSaving(false);
    }
  }

  async function runReplayIdempotencyAudit() {
    setSaving(true);
    setError(null);
    try {
      const response = await apiClient.get<GlmReplayIdempotencyAudit>('/api/referrals/admin/glm-replay-idempotency-audit');
  ...
  async function setProductionSignerEmergencyPause(paused: boolean) {
    setSaving(true);
    setError(null);
    try {
      const response = await apiClient.post<{ result?: ProductionSignerCheck }>('/api/referrals/admin/glm-production-signer/emergency-pause', {
        paused,
        reason: paused ? 'Admin emergency paused GLM production signer from crypto readiness panel.' : 'Admin resumed GLM production signer from crypto readiness panel.',
      });
      setProductionSignerCheck(response.data.result || { status: paused ? 'paused' : 'ok', emergency_paused: paused });
      await loadGlmTonReadiness();
    } catch (e: any) {
      setError(e?.response?.data?.detail || e?.message || 'Не удалось изменить emergency pause production signer');
    } finally {
      setSaving(false);
    }
  }

  async function saveProductionApprovals() {
    setSaving(true);
    setError(null);
    setProductionApprovalsResult(null);
    try {
      await apiClient.post('/api/referrals/admin/glm-production-approvals', productionApprovalForm);
      setProductionApprovalsResult('Production approvals сохранены');
      await loadGlmTonReadiness();
    } catch (e: any) {
  ...
  async function exportGlmTreasuryTurnoverCsv() {
    setSaving(true);
    setError(null);
    try {
      const response = await apiClient.get('/api/referrals/admin/glm-treasury-turnover/export.csv', {
      ...
      link.href = url;
      link.download = `glm-treasury-turnover-${new Date().toISOString().slice(0, 10)}.csv`;
      document.body.appendChild(link);
      link.click();
      link.remove();
      window.URL.revokeObjectURL(url);
    } catch (e: any) {
      setError(e?.response?.data?.detail || e?.message || 'Не удалось выгрузить treasury turnover CSV');
    } finally {
      setSaving(false);
    }
  }
  ...
  async function createHotWalletRefillApproval() {
    setSaving(true);
    setError(null);
    setHotWalletRefillResult(null);
    try {
      await apiClient.post('/api/referrals/admin/glm-hot-wallet-refill-approvals', {
        comment: `Approval request for refill: ${hotWalletRefillPlan?.refill_glm_amount || 0} GLM / ${hotWalletRefillPlan?.refill_ton_amount || 0} TON`,
      });
      setHotWalletRefillResult('Approval создан. Второй администратор должен подтвердить refill.');
      await loadGlmTonReadiness();
      await loadHotWalletRefillChecks();
      await loadGlmTreasuryTurnover();
    } catch (e: any) {
      setError(e?.response?.data?.detail || e?.message || 'Не удалось создать approval refill');
    } finally {
      setSaving(false);
    }
  }

  async function decideHotWalletRefillApproval(approvalId: string, approved: boolean) {
    setSaving(true);
    setError(null);
    setHotWalletRefillResult(null);
    try {
      await apiClient.post(`/api/referrals/admin/glm-hot-wallet-refill-approvals/${approvalId}/decision`, {
        approved,
        comment: approved ? 'Refill approved from admin UI.' : 'Refill rejected from admin UI.',
      });
      setHotWalletRefillResult(approved ? 'Approval подтвержден. Теперь можно пополнить через TON Connect.' : 'Approval отклонен.');
      await loadGlmTonReadiness();
      await loadHotWalletRefillChecks();
      await loadGlmTreasuryTurnover();
    } catch (e: any) {
      setError(e?.response?.data?.detail || e?.message || 'Не удалось обработать approval refill');
    } finally {
      setSaving(false);
    }
  }
  ...
  async function confirmHotWalletRefillInWallet() {
    ...
    try {
      ...
      if (hotWalletRefillApprovalRequired && !hotWalletMatchingApprovedRefillApproval?.id) {
        setHotWalletRefillTonResult('Для крупного refill нужен approved approval. Создайте approval и подтвердите его вторым администратором.');
        setSaving(false);
        return;
      }
      setHotWalletRefillTonResult('Готовим TON Connect транзакцию для treasury-кошелька...');
      const approvalQuery = hotWalletRefillApprovalRequired && hotWalletMatchingApprovedRefillApproval?.id
        ? `?approval_id=${encodeURIComponent(hotWalletMatchingApprovedRefillApproval.id)}`
        : '';
      const response = await apiClient.post<TonConnectTransactionPayload>(`/api/referrals/admin/glm-hot-wallet-refill-plan/ton-transaction${approvalQuery}`);
  ...
  return (
    <div className="min-w-0 space-y-6">
      <div className="rounded-md border border-slate-200 bg-white p-5 shadow-sm">
        <div className="text-sm font-medium uppercase tracking-wide text-slate-500">КРИПТО</div>
      ...
      <section className="rounded-md border border-slate-200 bg-white p-5 shadow-sm">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            {glmEffectiveness?.period?.operational_stats_start_at ? (
              <div className="mt-1 text-xs text-slate-500">
                Рабочая статистика: {label(glmEffectiveness.network || 'mainnet')} · с {dateRu(glmEffectiveness.period.operational_stats_start_at)}
              </div>
            ) : null}
          </div>
      ...
      <section id="ton-readiness" className="rounded-md border border-slate-200 bg-white p-5 shadow-sm scroll-mt-6">
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-3 xl:grid-cols-12">
          ...
          <Metric
            ...
            value={jettonDeploymentLabel(
              String(glmTonReadiness?.policy?.network || '').toLowerCase() === 'mainnet'
                ? glmTonReadiness?.policy?.status
                : glmTonReadiness?.artifact?.deployment_status || glmTonReadiness?.policy?.status,
        ...
        {productionSignerCheck ? (
          <div className={`mt-4 rounded-md border p-3 text-sm ${
            productionSignerCheck.status === 'ok'
              ? 'border-emerald-200 bg-emerald-50 text-emerald-800'
              : productionSignerCheck.status === 'warning'
                ? 'border-amber-200 bg-amber-50 text-amber-800'
                : 'border-rose-200 bg-rose-50 text-rose-800'
          }`}>
            <div className="font-semibold">
              Production signer: {label(productionSignerCheck.status || 'unknown')}
              {productionSignerCheck.http_status ? ` · HTTP ${productionSignerCheck.http_status}` : ''}
            </div>
            <div className="mt-1">
              mode={label(productionSignerCheck.mode || 'not_configured')} · endpoint={productionSignerCheck.health_endpoint_configured ? 'yes' : 'no'} · auth={productionSignerCheck.auth_configured ? 'yes' : 'no'}
            </div>
            <div className="mt-1">
              emergency pause={productionSignerCheck.emergency_paused ? 'yes' : 'no'}
              {productionSignerCheck.emergency_pause_source ? ` · source=${productionSignerCheck.emergency_pause_source}` : ''}
              {productionSignerCheck.emergency_pause_reason ? ` · ${productionSignerCheck.emergency_pause_reason}` : ''}
            </div>
            {productionSignerCheck.errors?.length ? <div className="mt-1">{productionSignerCheck.errors.join(' · ')}</div> : null}
          </div>
        ) : null}
        {glmTonReadiness?.go_no_go ? (
          <div className={`mt-4 rounded-md border p-4 ${
            ...
            <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
              <div>
                <div className="text-xs font-semibold uppercase tracking-wide text-slate-500">Public mainnet go/no-go</div>
                <div className={`mt-1 text-xl font-semibold ${glmTonReadiness.go_no_go.ready ? 'text-emerald-900' : 'text-amber-900'}`}>
                  {glmTonReadiness.go_no_go.ready ? 'GO' : 'Blocked'}
                </div>
                <div className="mt-1 text-sm text-slate-700">
                  Max per tx: {glmTonReadiness.go_no_go.max_amount_glm ?? '—'} GLM · daily cap: {glmTonReadiness.go_no_go.daily_limit_glm ?? '—'} GLM
                </div>
              </div>
              <Badge value={`${glmTonReadiness.go_no_go.blockers?.length || 0} blockers`} />
            </div>
            <div className="mt-4 grid gap-2 md:grid-cols-2 xl:grid-cols-3">
              {(glmTonReadiness.go_no_go.checks || []).map((item) => (
                <div
                  key={`go-no-go-${item.code}`}
                  className={`min-h-[92px] rounded-md border bg-white p-3 text-sm ${
                    item.ok ? 'border-emerald-200' : 'border-rose-200'
                  }`}
                >
                  <div className={`font-semibold ${item.ok ? 'text-emerald-800' : 'text-rose-800'}`}>
                    {item.ok ? 'OK' : 'BLOCK'} · {item.label || item.code}
                  </div>
                  <div className="mt-1 text-xs leading-5 text-slate-600">{item.message}</div>
                </div>
              ))}
            </div>
            {(glmTonReadiness.go_no_go.next_steps || []).length ? (
              <div className="mt-4 rounded-md border border-amber-200 bg-white p-3">
                <div className="text-xs font-semibold uppercase tracking-wide text-amber-700">Next steps</div>
                <div className="mt-2 grid gap-2 text-sm text-slate-700">
                  {(glmTonReadiness.go_no_go.next_steps || []).map((item, index) => (
                    <div key={`go-no-go-next-${index}`}>{index + 1}. {item}</div>
                  ))}
                </div>
              </div>
            ) : null}
        ...
        <div className="mt-4 rounded-md border border-slate-200 bg-slate-50 p-4">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              <div className="text-xs font-semibold uppercase tracking-wide text-slate-500">Production approvals</div>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <button
                onClick={() => void saveProductionApprovals()}
                disabled={saving}
                className="rounded-md border border-slate-300 bg-white px-4 py-2 text-sm font-medium text-slate-800 disabled:opacity-60"
              >
                Сохранить approvals
          ...
          <div className="mt-4 grid gap-3 lg:grid-cols-3">
            {[
              ['legal_approved', 'Legal approval', 'Оферта, utility, KYC/AML и учет проверены.'],
              ['security_approved', 'Security approval', 'Signer, treasury, лимиты и incident flow проверены.'],
              ['treasury_approved', 'Treasury approval', 'Политика банка, refill, burn и лимиты утверждены.'],
            ].map(([key, title, hint]) => (
              <label key={key} className="flex min-h-[92px] cursor-pointer flex-col gap-2 rounded-md border border-slate-200 bg-white p-3 text-sm text-slate-700">
                <span className="flex items-center gap-2 font-semibold text-slate-900">
                  <input
                    type="checkbox"
                    checked={Boolean(productionApprovalForm[key as keyof typeof productionApprovalForm])}
                    onChange={(event) => setProductionApprovalForm((prev) => ({ ...prev, [key]: event.target.checked }))}
                    className="h-4 w-4"
                  />
                  {title}
                </span>
          ...
          <textarea
            value={productionApprovalForm.comment}
            onChange={(event) => setProductionApprovalForm((prev) => ({ ...prev, comment: event.target.value }))}
          ...
          {replayIdempotencyAudit ? (
            <div className={`mt-3 rounded-md border p-3 text-sm ${
              replayIdempotencyAudit.status === 'ok'
                ? 'border-emerald-200 bg-emerald-50 text-emerald-800'
                : 'border-rose-200 bg-rose-50 text-rose-800'
            }`}>
              <div className="font-semibold">
                Replay/idempotency audit: {label(replayIdempotencyAudit.status || 'unknown')}
                {typeof replayIdempotencyAudit.issues_count === 'number' ? ` · issues ${replayIdempotencyAudit.issues_count}` : ''}
              </div>
              <div className="mt-1 text-xs">
                Bridge ops: {replayIdempotencyAudit.checked?.bridge_operations ?? 0} · transactions: {replayIdempotencyAudit.checked?.transactions ?? 0} · TON tx hashes: {replayIdempotencyAudit.checked?.ton_tx_hashes ?? 0}
              </div>
              <div className="mt-2 grid gap-2 md:grid-cols-2">
                {(replayIdempotencyAudit.checks || []).map((item) => (
                  <div key={item.code} className="rounded-md border border-current/20 bg-white/70 px-3 py-2">
                    <div className="font-semibold">{item.status === 'ok' ? 'OK' : 'FAIL'} · {item.code}</div>
                    <div className="mt-1 text-xs leading-5">{item.message}</div>
                  </div>
                ))}
              </div>
              {(replayIdempotencyAudit.issues || []).length ? (
                <div className="mt-2 max-h-48 overflow-auto rounded-md border border-current/20 bg-white/70 p-2 text-xs">
                  {(replayIdempotencyAudit.issues || []).slice(0, 8).map((item, index) => (
                    <div key={`replay-issue-${index}`} className="border-b border-current/10 py-1 last:border-b-0">
                      {item.code || 'issue'} · {item.tx_hash || item.operation_id || item.transaction_id || '—'}
                    </div>
                  ))}
                </div>
              ) : null}
        ...
        <div className="mt-4 rounded-md border border-slate-200 bg-slate-50 p-4">
          ...
          <div className="mt-4 rounded-md border border-slate-200 bg-white p-4">
            <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
              <div>
                ...
                <div className="mt-1 text-sm text-slate-600">
                  {hotWalletRefillPlan?.required
                    ? `Hot-wallet ниже лимита. Нужно отправить: ${hotWalletRefillPlan.refill_glm_amount || 0} GLM · ${hotWalletRefillPlan.refill_ton_amount || 0} TON gas`
                    : 'Пополнение не требуется по текущим лимитам.'}
                </div>
                <div className="mt-2 text-xs leading-5 text-slate-500">
                  {hotWalletRefillPlan?.status === 'blocked'
                      ? 'Сначала пополните treasury или уменьшите размер refill. В treasury сейчас недостаточно GLM/TON для пополнения hot-wallet.'
                ...
                {hotWalletRefillApprovalRequired ? (
                  <div className="mt-2 rounded-md border border-violet-200 bg-violet-50 px-3 py-2 text-xs text-violet-900">
                    Для этого refill нужен two-step approval: порог {hotWalletRefillPlan?.approval_policy?.glm_threshold ?? '—'} GLM или {hotWalletRefillPlan?.approval_policy?.ton_threshold ?? '—'} TON.
                    {hotWalletMatchingApprovedRefillApproval
                      ? ` Подтверждено: ${dateRu(hotWalletMatchingApprovedRefillApproval.created_at)}.`
                      : ' TON Connect будет заблокирован до подтверждения.'}
                  </div>
                ) : null}
              </div>
              <div className="flex flex-wrap items-center gap-2">
                <TonConnectButton />
                {hotWalletRefillApprovalRequired ? (
                  <button onClick={() => void createHotWalletRefillApproval()} disabled={saving || hotWalletRefillPlan?.status !== 'ready' || Boolean(hotWalletMatchingApprovedRefillApproval) || hotWalletPendingRefillApprovals.length > 0} className="rounded-md border border-violet-300 px-4 py-2 text-sm font-medium text-violet-700 disabled:opacity-60">Создать approval</button>
                ) : null}
                <button onClick={() => void confirmHotWalletRefillInWallet()} disabled={saving || hotWalletRefillPlan?.status !== 'ready' || (hotWalletRefillApprovalRequired && !hotWalletMatchingApprovedRefillApproval)} className="rounded-md border border-emerald-300 px-4 py-2 text-sm font-medium text-emerald-700 disabled:opacity-60">Пополнить через TON Connect</button>
            ...
            {hotWalletPendingRefillApprovals.length > 0 ? (
              <div className="mt-4 rounded-md border border-violet-200 bg-violet-50 p-3">
                <div className="text-xs font-semibold uppercase tracking-wide text-violet-700">Pending refill approval</div>
                <div className="mt-2 grid gap-2">
                  {hotWalletPendingRefillApprovals.slice(0, 3).map((item) => (
                    <div key={`approval-${item.id}`} className="flex flex-col gap-2 rounded-md border border-violet-200 bg-white p-3 text-xs text-slate-700 lg:flex-row lg:items-center lg:justify-between">
                      <div>
                        <div className="font-semibold text-slate-900">{item.refill_glm_amount ?? 0} GLM / {item.refill_ton_amount ?? 0} TON</div>
                        <div className="mt-1 text-slate-500">{dateRu(item.created_at)} · {item.comment || 'approval requested'}</div>
                      </div>
                      <div className="flex flex-wrap gap-2">
                        <button onClick={() => void decideHotWalletRefillApproval(item.id, true)} disabled={saving} className="rounded-md border border-emerald-300 px-3 py-2 font-medium text-emerald-700 disabled:opacity-60">Подтвердить</button>
                        <button onClick={() => void decideHotWalletRefillApproval(item.id, false)} disabled={saving} className="rounded-md border border-red-300 px-3 py-2 font-medium text-red-700 disabled:opacity-60">Отклонить</button>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-4">
              <Metric title="Долить GLM в hot-wallet" value={`${hotWalletRefillPlan?.refill_glm_amount ?? '—'}`} hint={`batch ${hotWalletRefillPlan?.target_glm ?? '—'}`} />
          ...
          <div className="mt-4 rounded-md border border-slate-200 bg-white p-4">
            <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
              <div>
                <div className="text-xs font-semibold uppercase tracking-wide text-slate-500">Treasury turnover</div>
                <div className="mt-1 text-sm text-slate-600">Входящие GLM от GLM Store, покупки баллов и GLM → баллы; исходящие refill в hot-wallet.</div>
                {glmTreasuryTurnover?.operational_stats_start_at ? (
                  <div className="mt-1 text-xs text-slate-500">
                    Рабочая статистика: {label(glmTreasuryTurnover.network || 'mainnet')} · с {dateRu(glmTreasuryTurnover.operational_stats_start_at)}
                  </div>
                ) : null}
              </div>
              <div className="flex flex-wrap gap-2">
                <button onClick={() => void exportGlmTreasuryTurnoverCsv()} disabled={saving} className="rounded-md border border-slate-300 px-3 py-2 text-xs font-medium text-slate-700 disabled:opacity-60">CSV export</button>
                <button onClick={() => void loadGlmTreasuryTurnover()} disabled={saving} className="rounded-md border border-slate-300 px-3 py-2 text-xs font-medium text-slate-700 disabled:opacity-60">Обновить оборот</button>
              </div>
            </div>
            <div className="mt-4 grid gap-3 md:grid-cols-5">
              <Metric title="Входящие GLM" value={`${glmTreasuryTurnover?.incoming_glm ?? '—'}`} hint={`${glmTreasuryTurnover?.period_days || 30} дней`} />
              <Metric title="Refill hot-wallet" value={`${glmTreasuryTurnover?.refill_glm ?? '—'} GLM`} hint={`${glmTreasuryTurnover?.refill_ton ?? 0} TON gas`} />
              <Metric title="Net treasury" value={`${glmTreasuryTurnover?.net_glm ?? '—'} GLM`} hint="входящие минус refill" />
              <Metric title="Refund required" value={`${glmTreasuryTurnover?.refund_required_glm ?? '—'} GLM`} hint={`${glmTreasuryTurnover?.refund_required_count ?? 0} операций`} />
              <Metric title="Treasury сейчас" value={`${glmTreasuryTurnover?.treasury_balance?.glm ?? hotWalletRefillPlan?.treasury_glm_balance ?? '—'} GLM`} hint={`${glmTreasuryTurnover?.treasury_balance?.ton ?? hotWalletRefillPlan?.treasury_ton_balance ?? '—'} TON`} />
            </div>
            {glmTreasuryTurnover?.by_source ? (
              <div className="mt-4 grid gap-3 md:grid-cols-3">
                {Object.entries(glmTreasuryTurnover.by_source).map(([source, payload]) => (
                  <div key={`turnover-source-${source}`} className="rounded-md border border-slate-200 bg-slate-50 p-3">
                    <div className="text-xs font-semibold uppercase tracking-wide text-slate-500">{label(source)}</div>
                    <div className="mt-2 text-xl font-semibold text-slate-900">{payload.amount_glm || 0} GLM</div>
                    <div className="mt-1 text-xs text-slate-500">{payload.count || 0} операций</div>
                  </div>
                ))}
              </div>
            ) : null}
            {(glmTreasuryTurnover?.items || []).length > 0 ? (
              <div className="mt-4">
                <DataTable
                  headers={['Дата', 'Тип', 'Источник', 'Сумма', 'Статус', 'Основание', 'TON tx']}
                  rows={(glmTreasuryTurnover?.items || []).slice(0, 10).map((item) => [
                    dateRu(item.created_at),
                    label(item.direction),
                    label(item.source),
                    item.direction === 'obligation'
                      ? `${item.amount_glm || 0} GLM к возврату`
                      : item.direction === 'outgoing'
                      ? `-${item.amount_glm || 0} GLM${item.amount_ton ? ` / ${item.amount_ton} TON` : ''}`
                      : `+${item.amount_glm || 0} GLM`,
                    <Badge key={`turnover-status-${item.id}`} value={item.status} />,
                    <div key={`turnover-title-${item.id}`} className="max-w-[280px] text-xs leading-5 text-slate-600">
                      <div className="font-medium text-slate-800">{item.title || label(item.source)}</div>
                      <div>{item.partner_name || item.comment || '—'}</div>
                    </div>,
                    <div key={`turnover-tx-${item.id}`} className="max-w-[260px] break-all text-xs text-slate-500">{item.tx_hash || '—'}</div>,
                  ])}
                />
              </div>
            ) : (
              <div className="mt-4 rounded-md border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-600">Оборота treasury за период пока нет.</div>
            )}
          </div>
      ...
      <section className="rounded-md border border-slate-200 bg-white p-5 shadow-sm">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          ...
          <div className="grid gap-2 sm:grid-cols-[0.8fr_1fr_1fr_auto]">
            ...
            <input value={glmRedemptionRefundTxHash} onChange={(event) => setGlmRedemptionRefundTxHash(event.target.value)} placeholder="TON refund tx hash" className="rounded-md border border-slate-300 px-3 py-2 text-sm" />
          ...
          <DataTable
            ...
            rows={glmRedemptions.map((item) => [
              ...
              item.ton_refund_required
                ...
                : item.ton_refund_status
                  ? `TON refund: ${label(item.ton_refund_status)}${item.ton_refund_tx_hash ? ` · ${item.ton_refund_tx_hash}` : ''}${item.ton_refund_verified_at ? ` · verified ${dateRu(item.ton_refund_verified_at)}` : ''}`
              ...
              <div key={`actions-${item.id}`} className="flex flex-wrap gap-2">
                ...
                <button disabled={saving || !item.ton_refund_required} onClick={() => void refundGlmRedemptionInWallet(item)} className="rounded border border-blue-200 px-3 py-2 text-xs font-medium text-blue-700 disabled:opacity-50">Вернуть TON</button>
                <button disabled={saving || !['wallet_request_prepared', 'submitted', 'sent', 'verification_failed'].includes(String(item.ton_refund_status || '')) || !glmRedemptionRefundTxHash.trim()} onClick={() => void settleGlmRedemptionTonRefund(item)} className="rounded border border-indigo-200 px-3 py-2 text-xs font-medium text-indigo-700 disabled:opacity-50">Проверить refund</button>
              </div>,
            ])}
          />
        </div>
        {glmRedemptionTonRefundResult ? (
          <div className="mt-3 rounded-md border border-blue-200 bg-blue-50 px-3 py-2 text-sm text-blue-800">{glmRedemptionTonRefundResult}</div>
        ) : null}
      </section>
